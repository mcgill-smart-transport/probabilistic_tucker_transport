function [fkten,final_loglik,bic] = ...
    lca_EM_aggregate_across(data,dataweight,nfactor,maxiter,cate,tol,plot_figure,rawten)
...
%         disp([iter,pi_weight]);
        bar(lambda{1,1}','stack'); ylim([0,1]); xlim([0,nfactor+1]);
        subplot(1,3,2);
...
fkten = ktensor(pi_weight',lambda);